...
<body id='dashboardcontent'>
        <div>
			...
			<div id="settingsMenu" class="hidden">
				<button><a id="editmode" class="menubar nodeco">Move Widgets</a></button>
				...
				<button><a type="button" class="menubar" onclick="toggleDisplay('EditDashboardDialog');">Edit Dashboard</a></button>
				<button><a class="menubar nodeco" href="actions/logout.php">Log Out</a></button>
				<button type="button" class="menubar" onclick='toggleDisplay("profileSettingsDialog");'>Profile Settings</button>
			</div>
		...
		<div id="EditDashboardDialog" class="white_content" style="right: initial !important; left:0 !important; width:450px; height: 350px;">
				...
				<button ID="btnSubmitDashboard">Save Dashboard</button>
				<a ID="btnDeleteDashboard" href="actions/deletedashboard.php?recid=<?php echo $dashboardid ?>"><button>DELETE DASHBOARD - NO CONFIRMATION CHECK WHEN CLICKED</button></a>
		</div>
		</form>
		<!-- Profile Settings Dialog -->
		<div id="profileSettingsDialog" class="white_content" style="display:none;z-index:1000;position:fixed;top:20%;left:35%;width:400px;">
			<button id="closeProfileSettingsDialogBtn" type="button" style="float:right;" onclick="closeProfileSettingsDialog(event)">Close</button>
			<h2>Profile Settings</h2>
			<button id="generateLoginTokenBtn" type="button">Generate Encrypted Login Token</button>
			<a id="downloadLoginToken" style="display:none;" download="login_token.dashify"></a>
			<div id="tokenStatus"></div>
		</div>
		<script>
		document.getElementById('generateLoginTokenBtn').onclick = function(e) {
			e.preventDefault();
			e.stopPropagation();
			fetch('actions/generate-login-token.php')
				.then(response => {
				...
				.then(blob => {
					...
					const a = document.getElementById('downloadLoginToken');
					a.href = url;
					a.style.display = 'inline';
					a.click();
					document.getElementById('tokenStatus').innerText = 'Token generated and downloaded.';
				})
				.catch((err) => {
					document.getElementById('tokenStatus').innerText = 'Failed to generate token: ' + err.message;
				});
		};
		</script>
		<script>
		function closeProfileSettingsDialog(e) {
			if (e) {
				e.preventDefault();
				e.stopPropagation();
			}
			document.getElementById('profileSettingsDialog').style.display = 'none';
		}
		</script>
