...
function doesDatabaseExist() {
	...
	$dbpath = $rootPath . '/Dashboardify/data/Dashboardify.s3db';
...
function selectquery_bind1($sql,$param1) {
	$dbpath = 'sqlite:' . rootdir() . '/data/Dashboardify.s3db';
	$db_file = new PDO($dbpath);
...
function getPDO_DBFile() {
	$rootPath = $_SERVER['DOCUMENT_ROOT'];
	$localdb = new PDO('sqlite:' . $rootPath . '/Dashboardify/data/Dashboardify.s3db');
	return $localdb;
}
...
function execquery($sql) {
	//$localdb = new PDO('sqlite:/Dashboardify/data/Dashboardify.s3db');
	$rootPath = $_SERVER['DOCUMENT_ROOT'];
	$localdb = new PDO('sqlite:' . $rootPath . '/Dashboardify/data/Dashboardify.s3db');
